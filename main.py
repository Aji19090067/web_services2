...
app.config['SQLALCHEMY_DATABASE_URI'] = "sqlite:///{}".format(
    os.path.join(app_dir, "simpan_img.db"))
...
if not os.path.exists(os.path.join(app_dir, 'simpan_img.db')):
    db.create_all()
...
def upload():
    ...
    if file.filename != '':
        try:
            ...
            return jsonify({'msge': 'Upload Failed :( nama harus unik, nama ini '+name+' sudah dipakai!'})
    else:
